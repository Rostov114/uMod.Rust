...
function Get-Dependencies {
    if ($access.ToLower() -ne "nosteam") {
        ...
        try {
            Start-Process dotnet -ArgumentList "$tools_dir\DepotDownloader.dll $login -app $appid -branch $branch $depot -os linux -dir $patch_dir -filelist $tools_dir\.references" -NoNewWindow -Wait
        } catch {
